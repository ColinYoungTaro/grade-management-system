<template>
    <a-layout>
        <a-layout-sider class="the_manage_class_sider" width="200" style="background: #fff">
            <a-menu
                    mode="inline"
                    :style="{ height: '100%', borderRight: 0 }"
            >
                <a-sub-menu key="/admin/appeals">
                <template #title>
                    <span><FormOutlined />
                        <router-link to="/admin/appeals" class="router-link-active">审核申请</router-link>
                    </span>
                </template>
            </a-sub-menu>
            </a-menu>
                ...
                <div align="center">
                    ...
                    >
                        <template v-slot:action="{ text, record }">
                            <a-space size="small">
                                <a-button type="primary" :style="record.appeal.status? {display:'none'} : {}">
                                    待处理
                                </a-button>
                            </a-space>
                            <a-space size="small">
                                <a-button type="primary" @click="checkApply(record)" :style="!(record.appeal.status==3)? {display:'none'} : {}">
                                    待审核
                                </a-button>
                            </a-space>
                            <a-space size="small">
                                <a-button type="default" :style="!(record.appeal.status==1||record.appeal.status==2||record.appeal.status==4)? {display:'none'} : {}">
                                    已处理
                                </a-button>
                            </a-space>
                        </template>
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const handleQuery = () => {
                loading.value = true;
                appealList.value = [];
                axios.get("/appeal/appeals").then((response) => {
                    loading.value = false;
            ...
            const handleApplyModalOk = () => {
                ...
                axios.post("/appeal/appeal_handle/admin", appealReq.value).then((response) => {
